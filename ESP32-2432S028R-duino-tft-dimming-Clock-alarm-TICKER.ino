...
#define SYSINFO_BTN_X 300
#define SYSINFO_BTN_Y 48
#define SYSINFO_BTN_W 15
#define SYSINFO_BTN_H 20
...
void desenButonSysInfo() {
  tft.fillRoundRect(SYSINFO_BTN_X, SYSINFO_BTN_Y, SYSINFO_BTN_W, SYSINFO_BTN_H, 8, ILI9341_DARKGREY);
  tft.setCursor(SYSINFO_BTN_X + 5, SYSINFO_BTN_Y + 10);
...
void afiseazaSystemInfo() {
  ...
  // CPU & frecventa
  tft.setCursor(10, y); 
  tft.printf("CPU: ESP32\n"); 
  y += 25;
  tft.setCursor(10, y);
  tft.printf("Freq: %lu MHz\n", ESP.getCpuFreqMHz());
  y += 25;

  // Memorie
  tft.setCursor(10, y);
  tft.printf("Mem Tot: %lu KB", ESP.getHeapSize()/1024);
  y += 20;
  tft.setCursor(10, y);
  tft.printf("Mem Lib: %lu KB", ESP.getFreeHeap()/1024);
  y += 25;

  // TFT
  tft.setCursor(10, y);
  tft.printf("TFT: ILI9341");
  y += 25;

  // Touch
  tft.setCursor(10, y);
  tft.printf("Touch: XPT2046");
  y += 25;

  // Tensiune
  tft.setCursor(10, y);
  float v = (float)analogRead(34)/4095.0 * 3.3; // exemplu pe pin ADC
  tft.printf("Tensiune: %.2f V", v);
  y += 25;
  ...
  tft.setCursor(10, y);
    ...
    tft.printf("Temp CPU: %.1f C", temp);
  #else
    tft.print("Temp CPU: N/A");
  #endif
  y += 25;
...
void updateDucoPriceOnly(float balance) {
  ...
  HTTPClient http;
  String url = String("https://server.duinocoin.com/v2/users/") + ducoUser;
  http.begin(url);
...
void verificaTouch() {
  ...
  if (readTouchPoint(p) && p.touched && calibrated) {
    beepDAC(2000, 40);  // bip scurt la orice atingere
        // Buton SYS INFO
      if (p.x > SYSINFO_BTN_X && p.x < (SYSINFO_BTN_X + SYSINFO_BTN_W) &&
          p.y > SYSINFO_BTN_Y && p.y < (SYSINFO_BTN_Y + SYSINFO_BTN_H)) {
        paginaCurenta = 4; // noua pagina
        afiseazaSystemInfo();
        return;
      }
...
animatieLogoDuino();
  ...
  wifiMulti.addAP("ssid1***** , ****Password1"); // preset cange-ssid1***** , ****Password1
  wifiMulti.addAP("ssid2***** , ****Password2"); // preset cange-ssid2***** , ****Password2
...
void loop() {
  ...
  int mapped=map(raw,ldrMin,ldrMax,255,10);
    ...
    // poți adăuga și actualizare ceas/LED backlight dacă vrei
        ...
        if (deserializeJson(doc, payload) == DeserializationError::Ok) {
            ...
            if (paginaCurenta == 1) {
                ...
                // TOTAL USD SUS
                tft.setCursor(180, 19);
                tft.setTextSize(1);
                tft.setTextColor(ILI9341_CYAN, ILI9341_BLACK);
                tft.print("$");
                tft.println(balanceUSD, 6);
